...
export default function Showcase() {
  ...
  const getPositionClass = (index) => {
    ...
    if (index === prev1) return "z-20 scale-95 opacity-60 -translate-x-40 -rotate-2 blur-sm";
    if (index === next1) return "z-20 scale-95 opacity-60 translate-x-40 rotate-2 blur-sm";
    if (index === prev2) return "z-10 scale-85 opacity-40 -translate-x-80 -rotate-3 blur-sm hidden md:block";
    if (index === next2) return "z-10 scale-85 opacity-40 translate-x-80 rotate-3 blur-sm hidden md:block";
    return "hidden";
  };
    ...
    <section id="showcase" className="bg-[#F4FFF8]">
      <div className="max-w-screen-xl mx-auto py-20 px-8 lg:px-20 text-center">
        ...
        <div className="relative flex items-center justify-center h-[550px]">
          {images.map((src, index) => (
            ...
            >
              <img
                ...
                width={270}
                height={470}
                className="rounded-xl shadow-xl cursor-pointer object-cover"
              />
            </div>
          ))}
          ...
          <button
            onClick={prevSlide}
            className="mr-5 lg:mr-0 absolute left-0 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow-md"
          >
            <IconChevronLeft size={24} className="text-[#2AD882]" />
          </button>
          <button
            onClick={nextSlide}
            className="absolute mr-5 lg:mr-0 right-0 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow-md"
          >
